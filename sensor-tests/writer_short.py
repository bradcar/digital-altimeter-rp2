...
# Code created by: Charlotte Swift  - THANKS!!
...
class Writer():
    ...
    def _newline(self):
        ...
        if margin < 0:  # Use < for less than comparison
    ...
    def _printchar(self, char, invert=False):
        ...
        if Writer.text_row + char_height > self.screenheight:  # Use > for greater than comparison
            if Writer.row_clip:
                return
            self._newline()
        if Writer.text_col + char_width > self.screenwidth:  # Use > for greater than comparison
            if Writer.col_clip:
                return
            else:
                self._newline()
